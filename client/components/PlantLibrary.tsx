...
import { PLANTS_DATABASE, PLANT_CATEGORIES, Plant, searchPlants } from '../data/plantsDatabase';
...
export const PlantLibrary: React.FC<PlantLibraryProps> = ({ 
  ...
  const [sortBy, setSortBy] = useState<'name' | 'difficulty' | 'rarity'>('name');
  const [selectedPlant, setSelectedPlant] = useState<Plant | null>(null);
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');

  // Filtered and sorted plants
  const filteredPlants = useMemo(() => {
    let plants = searchPlants(searchQuery, selectedCategory === 'all' ? undefined : selectedCategory);
    ...
    plants.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'difficulty':
          ...
          return difficultyOrder[a.growingConditions.difficulty] - difficultyOrder[b.growingConditions.difficulty];
        case 'rarity':
  ...
  const categoryStats = useMemo(() => {
    ...
    Object.keys(PLANT_CATEGORIES).forEach(category => {
      stats[category] = PLANTS_DATABASE.filter(plant => plant.category === category).length;
    });
    return stats;
  }, []);
    ...
    return (
      <Card className="h-full hover:shadow-lg transition-shadow cursor-pointer group">
        <CardHeader className="pb-3">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-2">
              <div className={`p-2 rounded-lg bg-opacity-20`} style={{ backgroundColor: PLANT_CATEGORIES[plant.category]?.color + '40' }}>
                <CategoryIcon className="w-4 h-4" style={{ color: PLANT_CATEGORIES[plant.category]?.color }} />
              </div>
              <div>
            ...
            <div className="flex flex-col gap-1">
              <Badge className={`text-xs ${difficultyColors[plant.growingConditions.difficulty]}`}>
        ...
        <CardContent className="pt-0">
          <p className="text-xs text-gray-600 line-clamp-2 mb-3">{plant.description}</p>
          
          {/* Quick info */}
          <div className="grid grid-cols-2 gap-2 text-xs mb-3">
            <div className="flex items-center gap-1">
              <Sun className="w-3 h-3 text-yellow-500" />
              <span className="capitalize">{plant.growingConditions.sunlight}</span>
            </div>
            <div className="flex items-center gap-1">
              <CloudRain className="w-3 h-3 text-blue-500" />
              <span className="capitalize">{plant.growingConditions.water}</span>
            </div>
            <div className="flex items-center gap-1">
          ...
          <div className="flex gap-2">
            ...
            <Button 
              size="sm" 
              className="flex-1 text-xs"
              style={{ backgroundColor: PLANT_CATEGORIES[plant.category]?.color }}
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">{PLANTS_DATABASE.length} medicinal plants</p>
        </div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-2">
          <Select value={selectedCategory} onValueChange={setSelectedCategory}>
            ...
            <SelectContent>
              <SelectItem value="all">All Categories ({PLANTS_DATABASE.length})</SelectItem>
          ...
          <Select value={sortBy} onValueChange={(value: 'name' | 'difficulty' | 'rarity') => setSortBy(value)}>
      ...
      {selectedCategory === 'all' && (
        <div className="grid grid-cols-2 gap-2">
          {Object.entries(PLANT_CATEGORIES).slice(0, 6).map(([key, category]) => {
            const Icon = categoryIcons[key] || Leaf;
            return (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-500">{categoryStats[key] || 0} plants</p>
                        </div>
                      </div>
                    </Card>
                  </TooltipTrigger>
      ...
      <div className="flex items-center justify-between text-sm text-gray-600">
        <span>
          ...
          {selectedCategory !== 'all' && ` in ${PLANT_CATEGORIES[selectedCategory]?.name}`}
        </span>
        {searchQuery && (
          ...
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                {React.createElement(categoryIcons[selectedPlant.category] || Leaf, {
                  className: "w-5 h-5",
                  style: { color: PLANT_CATEGORIES[selectedPlant.category]?.color }
                })}
                {selectedPlant.name}
                <Badge className={`ml-2 ${difficultyColors[selectedPlant.growingConditions.difficulty]}`}>
            ...
            <div className="space-y-6">
              ...
              <Tabs defaultValue="medicinal" className="w-full">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedPlant.activeCompounds.map((compound, index) => (
                        <Badge key={index} variant="outline">{compound}</Badge>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedPlant.partUsed.map((part, index) => (
                        <Badge key={index} className="bg-green-100 text-green-800">{part}</Badge>
                      ))}
                    </div>
                  </div>
                </TabsContent>
                    ...
                    <div>
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          <span>Sunlight:</span>
                          <Badge variant="outline">{selectedPlant.growingConditions.sunlight}</Badge>
                        </div>
                        <div className="flex justify-between">
                          <span>Water:</span>
                          <Badge variant="outline">{selectedPlant.growingConditions.water}</Badge>
                        </div>
                        <div className="flex justify-between">
                          <span>Soil:</span>
                          <span className="text-right text-gray-600">{selectedPlant.growingConditions.soil}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Climate:</span>
                          <div className="flex gap-1">
                            {selectedPlant.growingConditions.climate.map((climate, index) => (
                              <Badge key={index} variant="outline" className="text-xs">{climate}</Badge>
                            ))}
                          </div>
                        </div>
                      </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {selectedPlant.companionPlants.map((companion, index) => (
                          <Badge key={index} className="bg-green-100 text-green-800">{companion}</Badge>
                        ))}
                      </div>
                    </div>
                  )}
                ...
                <TabsContent value="preparation" className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      {selectedPlant.uses.culinary && <Badge className="bg-orange-100 text-orange-800">Culinary</Badge>}
                      {selectedPlant.uses.medicinal && <Badge className="bg-red-100 text-red-800">Medicinal</Badge>}
                      {selectedPlant.uses.aromatic && <Badge className="bg-purple-100 text-purple-800">Aromatic</Badge>}
                      {selectedPlant.uses.decorative && <Badge className="bg-pink-100 text-pink-800">Decorative</Badge>}
                    </div>
                  </div>
                </TabsContent>
                  ...
                  <div>
                    ...
                    {selectedPlant.contraindications.length > 0 ? (
                      <div className="space-y-2">
                        {selectedPlant.contraindications.map((warning, index) => (
                      ...
                      <p className="text-sm text-gray-600 bg-green-50 p-3 rounded">
                        No specific contraindications noted. Always consult healthcare providers before medicinal use.
                      </p>
                    )}
                  </div>
                  
                  <div className="bg-blue-50 p-4 rounded border border-blue-200">
                    <h4 className="font-semibold mb-2 text-blue-800">General Safety Guidelines</h4>
                    <ul className="text-sm text-blue-700 space-y-1">
                      <li>• Always consult with a healthcare provider before using plants medicinally</li>
              ...
              <div className="flex justify-between items-center">
                ...
                <Button 
                  ...
                  style={{ backgroundColor: PLANT_CATEGORIES[selectedPlant.category]?.color }}
                >
