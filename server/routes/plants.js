...
router.get('/species', optionalAuth, async (req, res) => {
  try {
    const { category, careLevel, sunlight, search, limit = 20, offset = 0 } = req.query;
...
router.get('/species/:id/care', async (req, res) => {
  try {
    ...
    const careInstructions = {
      watering: {
        frequency: species.watering === 'high' ? 'Daily' : 
                  species.watering === 'medium' ? 'Every 2-3 days' : 'Weekly',
        amount: species.watering === 'high' ? 'Generous watering' :
                species.watering === 'medium' ? 'Moderate watering' : 'Light watering',
        tips: [
      ...
      sunlight: {
        ...
        hours: species.sunlight === 'full' ? '6+ hours direct sunlight' :
               species.sunlight === 'partial' ? '3-6 hours sunlight' : 'Indirect light',
        placement: species.sunlight === 'full' ? 'South-facing location' :
                   species.sunlight === 'partial' ? 'East or west-facing location' : 'North-facing or shaded area'
      },
      soil: {
...
router.get('/categories', async (req, res) => {
  try {
    ...
    const formattedCategories = categories.map(cat => ({
      ...
      displayName: cat.category.charAt(0).toUpperCase() + cat.category.slice(1) + 's'
    }));
...
router.get('/health-stats', authenticateToken, async (req, res) => {
  try {
    ...
    const stats = {
      ...
      readyToHarvest: plants.filter(p => p.growthStage >= 80 && 
        (!p.lastHarvested || 
         (new Date() - new Date(p.lastHarvested)) > 7 * 24 * 60 * 60 * 1000)).length,
      averageHealth: plants.length > 0 ? 
        Math.round(plants.reduce((sum, p) => sum + p.health, 0) / plants.length) : 0,
      averageWaterLevel: plants.length > 0 ? 
        Math.round(plants.reduce((sum, p) => sum + p.waterLevel, 0) / plants.length) : 0,
      averageGrowthStage: plants.length > 0 ? 
        Math.round(plants.reduce((sum, p) => sum + p.growthStage, 0) / plants.length) : 0
    };
...
router.patch('/:plantId/status', authenticateToken, async (req, res) => {
  try {
    ...
    if (waterLevel !== undefined) updateData.waterLevel = Math.max(0, Math.min(100, waterLevel));
    if (growthStage !== undefined) updateData.growthStage = Math.max(0, Math.min(100, growthStage));
...
router.post('/:plantId/simulate-growth', authenticateToken, async (req, res) => {
  try {
    const { plantId } = req.params;
    ...
    const newGrowthStage = Math.min(100, plant.growthStage + (growthRate * days));
    const newWaterLevel = Math.max(0, plant.waterLevel - (waterDecayRate * days));
    const newHealth = Math.max(0, Math.min(100, plant.health + (healthChangeRate * days)));
