...
if (!fs.existsSync(nodeModulesPath)) {
  ...
  const npmInstall = spawn('npm', ['install'], { cwd: rootDir, stdio: 'inherit' });
...
function startDatabase() {
  ...
  const prismGen = spawn('npx', ['prisma', 'generate'], { cwd: rootDir, stdio: 'inherit' });
  ...
  prismGen.on('close', (code) => {
    if (code === 0) {
      ...
      const prismaPush = spawn('npx', ['prisma', 'db', 'push'], { cwd: rootDir, stdio: 'inherit' });
      ...
      prismaPush.on('close', (code) => {
        if (code === 0) {
          ...
          console.log('⚠️  Database push failed. Continuing with server startup...');
          startServer();
        }
      });
    } else {
...
function seedDatabase() {
  ...
  const seed = spawn('npm', ['run', 'db:seed'], { cwd: rootDir, stdio: 'inherit' });
...
function startServer() {
  ...
  const server = spawn('npm', ['run', 'server'], { cwd: rootDir, stdio: 'inherit' });
